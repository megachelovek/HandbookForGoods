...
namespace GoodsHandbookMalchikovPavlov.Validators
{
    ...
    public static class CommonValidator
    {
        public static ValidateResult ValidateNonNegativeIntInRange(string value, int min, int max)
        {
            ...
            try
            {
                parsed = int.Parse(value, NumberStyles.None | NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite);
            }
            catch
            {
        ...
        public static ValidateResult ValidateNonNegativeFloatInRange(string value, float min, float max, int digitsAfterDecimalPoint)
        {
            float parsed;
            try
            {
                parsed = float.Parse(value, NumberStyles.None | NumberStyles.AllowDecimalPoint | NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite);
            }
            catch
            {
            ...
            Regex regex = new Regex($"{decimalSeparator}[0-9]{{{digitsAfterDecimalPoint}}}\\s*$");
        ...
        {
            if (Misc.FindString(value, arr) > -1)
            {
                return ValidateResult.Ok;
            }
            return ValidateResult.OutsideRange;
        ...
        public static string GetErrorMessageNonNegativeInteger(ValidateResult error, int min, int max, string name)
        {
            switch (error)
            {
                ...
                case ValidateResult.OutsideRange:
                    {
                        return $"\"{name}\" should be a non-negative integer number from {min} to {max}" + Environment.NewLine;
                    }
                default:
                    {
        ...
        public static string GetErrorMessageNonNegativeFloat(ValidateResult error, float min, float max, int digitsAfterDecimalPoint, string name)
        {
            switch (error)
            {
                ...
                case ValidateResult.OutsideRange:
                    {
                        return $"\"{name}\" should be a non-negative floating point number from {min.ToString($"F{digitsAfterDecimalPoint}")} to {max.ToString($"F{digitsAfterDecimalPoint}")}" + Environment.NewLine;
                    }
                case ValidateResult.DigitsAfterDecimalPointOutsideRange:
                    {
                        return $"\"{name}\" should be a non-negative floating point number with {digitsAfterDecimalPoint} digits after decimal point" + Environment.NewLine;
                    }
                default:
                    {
                        throw new ArgumentException();
